...
const DEFAULT_FALLBACK = ['anysearch', 'deepseek-official'];
...
function normalizeConfig(config) {
    ...
    if (fallback.length === 0)
        fallback.push(...DEFAULT_FALLBACK);
    const searxngInstances = Array.isArray(config?.searxngInstances)
        ? config.searxngInstances.filter((item) => typeof item === 'string' && item.length > 0)
        : [];
...
function readConfigFile() {
    try {
        ...
        const searxngInstances = Array.isArray(parsed.searxngInstances)
            ? parsed.searxngInstances.filter((item) => typeof item === 'string' && item.length > 0)
            : [];
...
function parseCredentialsRefs(file) {
    try {
        const text = readFileSync(file, 'utf8');
        const out = {};
...
function resolveApiKey(name) {
    ...
    if (name === 'ANYSEARCH_API_KEY' && state.config.anysearchApiKey)
        return state.config.anysearchApiKey;
    if (name === 'DEEPSEEK_API_KEY' && state.config.deepseekApiKey)
        return state.config.deepseekApiKey;
    if (process.env[name])
...
function withCredentialMasks(config) {
    const next = { ...config };
...
/* Backends                                                            */
/* ------------------------------------------------------------------ */
async function searchDeepSeekOfficial(query, maxResults, signal) {
    ...
    for (const block of blocks) {
        ...
        for (const item of block.content) {
            if (!item || typeof item !== 'object' || item.type !== 'web_search_result' || !item.url)
                continue;
            const itemRecord = item;
            if (sources.some(source => source.url === itemRecord.url))
                continue;
            sources.push({
                url: itemRecord.url,
...
async function fetchHtml(url, signal, acceptLang = ACCEPT_LANG) {
    let response;
    try {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), 12000);
...
async function searchBing(query, maxResults, signal) {
...
async function searchDdg(query, maxResults, signal) {
...
async function searchSearxng(query, maxResults, instances, signal) {
    const errors = [];
...
/* Router provider                                                     */
/* ------------------------------------------------------------------ */
async function runBackend(backend, query, maxResults, searxngInstances, signal) {
    switch (backend) {
...
function createRouterProvider() {
    return {
        ...
        async search(request, signal) {
            ...
            const chain = [config.provider, ...config.fallback.filter(backend => backend !== config.provider)];
            let lastError = null;
...
const statusTool = {
    ...
    description: 'Show the current web search backend of the search router, its fallback chain, all available backends, and the last failure if any.',
    parameters: {
        type: 'object',
        additionalProperties: false,
        properties: {},
...
const switchTool = {
    ...
    description: 'Switch the current web search backend immediately. Choose one of: deepseek-official (DeepSeek official search, needs DEEPSEEK_API_KEY), anysearch (AnySearch, anonymous or ANYSEARCH_API_KEY), bing (free, default), ddg / ddg-lite (free DuckDuckGo), searxng (free meta-search). The next web_search call uses the new backend without restart.',
    parameters: {
        type: 'object',
        additionalProperties: false,
        properties: {
    ...
    try {
        ctx.inject(['settings'], (sctx) => {
            ...
            sctx.settings.installSection(ctx, SETTINGS_NS, RouterConfigSchema, state.config, {
                setSource: (source) => {
                    currentConfig = () => normalizeConfig(source());
